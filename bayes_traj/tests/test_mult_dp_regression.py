...
def test_update_u():
    ...
    sig = 0.2
    np.random.seed(42)
    eps = sig*np.random.randn(3*num_dom_pts)

    # Ground truth param values for three "individuals" (columns are intercept
    # and slope)
    params = np.zeros((3, 2))
    x = np.linspace(0, 100, num_dom_pts)
    params[0, :] = np.array([5, -5/100])
    params[1, :] = np.array([7, -7/100])
    params[2, :] = np.array([3, -3/100])
    y = params[0, 0] + params[0, 1]*x + eps[0:num_dom_pts]
    y1 = params[1, 0] + params[1, 1]*x + eps[num_dom_pts:2*num_dom_pts]
    y2 = params[2, 0] + params[2, 1]*x + eps[2*num_dom_pts:3*num_dom_pts]
    ...
    X_mat = np.ones((N, M))
    X_mat[0:num_dom_pts, 1] = x
    X_mat[num_dom_pts:2*num_dom_pts, 1] = x
    X_mat[2*num_dom_pts:3*num_dom_pts, 1] = x
    
    Y_mat = np.zeros(3*num_dom_pts)
    Y_mat[0:num_dom_pts] = y
    Y_mat[num_dom_pts:2*num_dom_pts] = y1
    Y_mat[2*num_dom_pts:3*num_dom_pts] = y2
    
    mm.M_ = M
    mm.D_ = D
    mm.X_ = torch.from_numpy(X_mat)
    mm.Y_ = torch.from_numpy(Y_mat).unsqueeze(-1)
    mm.predictor_names_ = ['intercept', 'x']
    mm.target_names_ = ['y']
    
    mm.sig_trajs_ = torch.zeros(K, dtype=bool)
    mm.sig_trajs_[0] = True
    
    R = np.zeros([N, K])
    R[:, 0] = 1
    mm.R_ = torch.from_numpy(R)
    
    w_mu = torch.zeros([M, D, K])
    w_mu[:, 0, 0] = torch.tensor([params[0, 0], params[0, 1]])
    mm.w_mu_ = w_mu
    
    df = pd.DataFrame()
    df['x'] = X_mat[:, 1]
    df['intercept'] = X_mat[:, 0]
    df['y'] = Y_mat[:]
    df['id'] = ['a']*num_dom_pts + ['b']*num_dom_pts + ['c']*num_dom_pts
    
    mm.df_ = df
    mm.gb_ = mm.df_.groupby('id')
    
    lambda_a = torch.ones([D, K])
    lambda_b = torch.ones([D, K])
    lambda_a[0, 0] = 377
    lambda_b[0, 0] = 45
    mm.lambda_a_ = lambda_a
    mm.lambda_b_ = lambda_b
    
    group_first_index = np.zeros(N, dtype=bool)
    group_first_index[0] = True
    group_first_index[num_dom_pts] = True
    group_first_index[2*num_dom_pts] = True
    mm.group_first_index_ = group_first_index

    ranefs = ['intercept', 'x']
    mm.ranef_indices_ = np.array([True, True])
    num_ranefs = np.sum(mm.ranef_indices_)

    G = mm.gb_.ngroups

    X_outer = torch.einsum('ij,ik->ijk', mm.X_[:, mm.ranef_indices_],
                           mm.X_[:, mm.ranef_indices_])
    mm.G_r_r_ = torch.zeros([G, num_ranefs, num_ranefs])
    inc = 0
    for ii, vv in enumerate(mm.gb_.groups.values()):
        mm.G_r_r_[ii, :, :] = torch.sum(X_outer[vv, :, :], axis=0)
    ...
    # Test 1
    #---------------------------------------------------------------------------
    mm.Sig0_['y'] = 0.000001*torch.tensor([[1, 0], [0, 1]], dtype=float)    
    ...
    # small, meaning that the random effects are not expected to significantly
    # deviate from 0.
    assert torch.all(mm.u_mu_[:, 0, 0, :] < 5e-4)

    #---------------------------------------------------------------------------
    # Test 2
    #---------------------------------------------------------------------------
    mm.Sig0_['y'] = 1e10*torch.tensor([[1, 0], [0, 1]], dtype=float)    
    ...
    # We have made the covariance matrix over the random effects extremely
    # large, meaning that the random effects are expected to be reasonably close
    # to the per-individual parameters.
    assert torch.all(torch.isclose(mm.u_mu_[:, 0, 0, :] + params,
        torch.from_numpy(params[0,:]).unsqueeze(0), rtol=1e-1))

    #---------------------------------------------------------------------------
    # Test 3
    #---------------------------------------------------------------------------
    mm.Sig0_['y'] = torch.tensor([[1, 0], [0, 1]], dtype=float)    
    ...
    # precision.
    assert torch.all((torch.abs(large_prec_ranefs) - \
                      torch.abs(small_prec_ranefs))[1:3, :] > 0)
    
    
    #plt.scatter(X_mat[:, 1], Y_mat[:])
    #plt.plot([0, 100], a + b*np.array([0, 100]), c='k')

    #a_tmp = a + mm.u_mu_[0, 0, 0, 0]
    #b_tmp = b + mm.u_mu_[0, 0, 0, 1]
    #plt.plot([0, 100], a_tmp + b_tmp*np.array([0, 100]), c='r')

    #a_tmp = a + mm.u_mu_[1, 0, 0, 0]
    #b_tmp = b + mm.u_mu_[1, 0, 0, 1]
    #plt.plot([0, 100], a_tmp + b_tmp*np.array([0, 100]), c='r')

    #a_tmp = a + mm.u_mu_[2, 0, 0, 0]
