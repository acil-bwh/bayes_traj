...
args = ArgumentParser()
args.add_argument('--model', help='Pickled MultDPRegression object that has \
  been fit to data and from which we will extract information to produce an \
  updated prior file', dest='model', default=None)
args.add_argument('--prior', help='Output pickle file containing updated prior \
  settings', dest='prior', default=None)
...
for m in range(M):
    for d in range(D):
        samples = []
        for t in traj_ids:
            samples.append(mm.w_mu_[m, d, t] + np.sqrt(mm.w_var_[m, d, t])*\
                           np.random.randn(num_traj_samples[t]))

        w_mu0_post[m, d] = np.mean(np.hstack(samples))
        w_var0_post[m, d] = np.var(np.hstack(samples))

# For precision parameters, we'll use a similar sample-based procedure as was
# done for the coefficients
...
for d in range(D):
    ...
    lambda_b0_post[d] = np.mean(np.hstack(samples))/np.var(np.hstack(samples))

pickle.dump({'w_mu0': w_mu0_post, 'w_var0': w_var0_post,
             'lambda_a0': lambda_a0_post, 'lambda_b0': lambda_b0_post,
             'alpha': mm.alpha_}, open(op.prior, 'wb'))
